...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<RestResponse<Void>> handleException(Exception e, HttpServletRequest request) {
        log.error("系统内部异常，请求路径: {}，错误信息: ", request.getRequestURI(), e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(RestResponse.fail(HttpStatus.INTERNAL_SERVER_ERROR.value(), "认证服务暂时不可用，请稍后重试"));
    }
